...
import PopularTvSeriesCard from "./PopularTvSeriesCard";
...
const PopularTvSeries = () => {
  ...
  const fetchPopularTvSeries = async (page) => {
    try {
        let searchParams = `language=en-US&api_key=${process.env.NEXT_PUBLIC_MOVIE_API_KEY}&page=${page}`;
        if (searchQuery) {
          searchParams += `&query=${searchQuery}`;
        }
        const response = await fetch(`${API_URL}tv/top_rated?${searchParams}`);
        if (response.ok) {
  ...
  useEffect(() => {
    fetchPopularTvSeries(currentPage);
  ...
  const renderPopularTvSeries = () => (
    <AnimatePresence mode="wait">
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4" key={currentPage}>
        {popularTvSeries
          .filter((series) => series.name.toLowerCase().includes(searchQuery.toLowerCase()))
          .map((series) => (
            <div key={series.id}>
              <PopularTvSeriesCard series={series} />
            </div>
          ))}
      </div>
    </AnimatePresence>
