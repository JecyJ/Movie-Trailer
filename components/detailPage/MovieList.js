'use client'


import { useEffect, useState } from "react";
import MovieListCard from "./MovieListCard";



const MovieList = () => {
  const API_URL = "https://api.themoviedb.org/3/";
  const [movieList, setMovieList] = useState([]);



  const fetchUpcomingMovies = async (searchKey) => {
    try {
      const response = await fetch(
        `${API_URL}discover/movie?include_adult=false&include_video=false&language=en-US&page=1&sort_by=popularity.desc&api_key=${process.env.NEXT_PUBLIC_MOVIE_API_KEY}`
      );
      if (response.ok) {
        const data = await response.json();
        setMovieList([...movieList, ...data.results]);
      } else {
        console.error("Failed to fetch movies:", response.status);
      }
    } catch (error) {
      console.error("Error:", error);
    }
  };

  useEffect(() => {
    fetchUpcomingMovies();
  }, []);


  const renderMovieList = () => (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
      {movieList.map((movie) => (
        <div key={movie.id} >
          <MovieListCard movie={movie} />
        </div>
      ))}
    </div>
  );


  return (
    <section className="w-full h-auto my-3">
      <div className="max-w-[400px] sm:max-w-[600px] md:max-w-[800px] lg:max-w-[1200px] m-auto">
        <h1 className="text-xl text-center text-white font-bold pb-5">Upcoming Movies</h1>
        <div className="flex flex-col items-center justify-center space-x-4">
          <div className="flex overflow-x-auto">
            {renderMovieList()}
          </div>                    
        </div>
      </div>
    </section>
  )
}

export default MovieList;












// 'use client'


// import Link from "next/link";
// import { useEffect, useState } from "react";
// import { BiLeftArrow, BiRightArrow } from "react-icons/bi";
// import { motion } from "framer-motion";
// import MovieListCard from "./MovieListCard";



// const MovieList = () => {
//   const API_URL = "https://api.themoviedb.org/3/";
//   const [movieList, setMovieList] = useState([]);
// //   const [currentIndex, setCurrentIndex] = useState(0);
// const [searchKey, setSearchKey] = useState('')



//   const fetchUpcomingMovies = async (searchKey) => {
//     try {
//       const type = searchKey ? "search" : "discover/movie?include_adult=false&include_video=false&language=en-US&page=1-32&sort_by=popularity.desc"
//       const response = await fetch(
//         `${API_URL}${type}&api_key=${process.env.NEXT_PUBLIC_MOVIE_API_KEY}`
//       );
//       if (response.ok) {
//         const data = await response.json();
//         setMovieList([...movieList, ...data.results]);
//       } else {
//         console.error("Failed to fetch movies:", response.status);
//       }
//     } catch (error) {
//       console.error("Error:", error);
//     }
//   };

//   useEffect(() => {
//     fetchUpcomingMovies();
//   }, []);


// //   const handlePrev = () => {
// //     setCurrentIndex((prevIndex) => (prevIndex === 0 ? upcomingMovies.length - 1 : prevIndex - 1));
// //   };

// //   const handleNext = () => {
// //     setCurrentIndex((prevIndex) => (prevIndex === upcomingMovies.length - 1 ? 0 : prevIndex + 1));
// //   };

//   const renderMovieList = () => (
//     <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
//       {movieList.map((movie) => (
//         <div key={movie.id} >
//           <MovieListCard movie={movie} />
//         </div>
//       ))}
//     </div>
//   );
  


//   //   const renderUpcomingMovies = () => (
//   //   <div className="flex space-x-3">
//   //     {upcomingMovies.map((movie, index) => (
//   //       <motion.div
//   //         key={movie.id}
//   //         className={`w-full h-auto ${index === currentIndex ? "flex" : "hidden"}`}
//   //         initial={{ opacity: 0 }}
//   //         animate={{ opacity: index === currentIndex ? 1 : 0 }}
//   //         transition={{ duration: 0.3 }}
//   //       >
//   //         <UpcomingMovieCard movie={movie} />
//   //       </motion.div>
//   //     ))}
//   //   </div>
//   // );

//   const searchMovies = (e) => {
//     e.preventDefault()
//     fetchUpcomingMovies(searchKey)
//   }


//   return (
//     <section className="w-full h-auto my-3">
//         <div className="max-w-[400px] sm:max-w-[600px] md:max-w-[800px] lg:max-w-[1200px] m-auto">
//             <h1 className="text-xl text-center text-white font-bold pb-5">Upcoming Movies</h1>
//             <form className="space-x-2 py-3" onSubmit={searchMovies}>
//               <input className="py-1 rounded-lg bg-slate-400" type="text" onChange={(e) => setSearchKey(e.target.value)} />
//               <button className="border text-white rounded-lg py-1 px-3" type={"submit"}>Search</button>
//             </form>
//             <div className="flex flex-col items-center justify-center space-x-4">
//                 <div className="flex overflow-x-auto">
//                     {renderMovieList()}
//                 </div>
//                 <div className="flex space-x-3">
//                 {/* <motion.div
//                     className="flex justify-center items-center"
//                     style={{ zIndex: 1 }}
//                     // onClick={handlePrev}
//                 >
//                     <BiLeftArrow size={30} className="text-orange-600" />
//                 </motion.div>
//                 <p className="text-white">1</p>
//                 <motion.div
//                     className="flex justify-center items-center"
//                     style={{ zIndex: 1 }}
//                     // onClick={handleNext}
//                 >
//                     <BiRightArrow size={30} className="text-orange-600" />
//                 </motion.div> */}
//                 </div>            
//             </div>
//         </div>
//     </section>
//   )
// }

// export default MovieList;